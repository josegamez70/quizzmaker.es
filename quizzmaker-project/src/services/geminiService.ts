...
export async function generateQuizFromImageAndText(files: File[], numQuestions: number): Promise<Question[]> {
  try {
    ...
    const model = genAI.getGenerativeModel({ 
        model: "gemini-1.5-flash-latest", // Usamos el modelo correcto y estable
        generationConfig,
        safetySettings
    });
    ...
    if (error instanceof Error && 'message' in error) {
        if ((error as any).message.includes('404')) {
            throw new Error(`El modelo de IA especificado no se encontró. Por favor, contacta al soporte.`);
        }
        if ((error as any).message.includes('API key not valid')) {
            throw new Error('Tu clave de API no es válida. Revisa la configuración en Netlify.');
        }
    }
