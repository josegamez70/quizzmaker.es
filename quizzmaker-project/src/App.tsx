  ...
  const renderContent = () => {
    switch (appState) {
      case AppState.GENERATING:
        return <Loader text="Generando tu cuestionario... La IA está pensando." />;
      case AppState.QUIZ:
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-4 sm:p-6 lg:p-8 print:bg-white print:p-0 print:block">
      <header className="w-full max-w-5xl mx-auto mb-6 flex items-center justify-between print:hidden">
        ...
        <div className="flex items-center gap-4">
            <span className="text-sm text-gray-300 hidden sm:block" title={profile?.username || session.user.email}>
                Hola, <span className="font-semibold">{profile?.username || session.user.email?.split('@')[0]}</span>
            </span>
            <button onClick={handleShowSaved} title="Mis Cuestionarios" className="flex items-center gap-2 p-3 rounded-md text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">
                <BookmarkIcon className="w-5 h-5"/>
                <span className="hidden md:inline text-sm font-medium">Guardados</span>
            </button>
            <button onClick={handleLogout} title="Cerrar Sesión" className="flex items-center gap-2 p-3 rounded-md text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">
...
export function App() {
  ...
  if (session && authEvent === 'PASSWORD_RECOVERY') {
      return <UpdatePasswordView onPasswordUpdated={handlePasswordUpdated} />;
  }

  return session ? <MainApp session={session} forceLogout={forceLogout} /> : <AuthView />;
}
